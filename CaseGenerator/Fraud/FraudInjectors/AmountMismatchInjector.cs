...
public class AmountMismatchInjector : IFraudInjector
{
    ...
    {
        FactType factTypeMod = FactType.Amount;
        ...
        // Modify a fact
        if (decimal.TryParse(secondEvFact.Values["Amount"].ToString(), out decimal result))
        {
            // Conversion succeeded, use 'result' here
            firstEvFact.Values["Amount"] = Math.Round(result + GenerateRandom.Money(100, 1000), 2).ToString();
        }
        else
        {
            firstEvFact.Values["Amount"] = "0";
        }
        ...
        return new Contradiction
        {
            ...
            FraudType = FraudType.AmountMismatch,

            Description = "Sales Amount mismatch."
        };
    }
}